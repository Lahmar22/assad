...
$visites = $conn->query($sqlVisiteGuid);
                    ...
                    <tbody class="divide-y divide-gray-200">
                        <?php while ($row = $resultVisiteGuid->fetch_assoc()) { ?>
                            <tr class="hover:bg-gray-50 transition">
                                ...
                                <td class="px-6 py-4 text-sm text-gray-700"><?= $row["titre"] ?></td>
                                <td class="px-6 py-4 text-sm text-gray-700"><?= $row["dateheure"] ?></td>
                                <td class="px-6 py-4 text-sm text-gray-700"><?= $row["langue"] ?></td>
                                <td class="px-6 py-4 text-sm text-gray-700"><?= $row["capacite_max"] ?></td>
                                <td class="px-6 py-4 text-sm text-gray-700"><?= $row["duree"] ?></td>
                                <td class="px-6 py-4 text-sm text-gray-700"><?= $row["prix"] ?> MAD</td>
                                <td class="px-6 py-4 text-sm text-gray-700">
                                    <form action="../controller/update_statusVisiteGuid.php" method="POST">
                                        <input type="hidden" name="id_visiteGuid" value="<?= $row['id'] ?>">

                                        <select name="statut"
                                            onchange="this.form.submit()"
                                            class="px-3 py-1 rounded-md text-sm font-medium <?= $row['statut'] === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700' ?>">

                                            <option value="Active" <?= $row['statut'] === 'Active' ? 'selected' : '' ?>>
                                                Activer
                                            </option>
                                            <option value="Inactive" <?= $row['statut'] === 'Inactive' ? 'selected' : '' ?>>
                                                Désactiver
                                            </option>
                                        </select>
                                    </form>
